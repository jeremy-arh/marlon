...
export default function SpecialtyForm({ specialty, onSuccess, onCancel }: SpecialtyFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = specialty ? `/api/admin/specialties/${specialty.id}` : '/api/admin/specialties';
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="sticky bottom-0 mt-8 flex flex-col sm:flex-row gap-4 border-t border-gray-200 bg-white pt-6 -mx-6 px-6">
        ...
        >
          {loading ? 'Sauvegarde...' : specialty ? 'Modifier' : 'Créer'}
        </Button>
        <Button
          type="button"
          onClick={onCancel}
