...
export async function PUT(
  ...
  try {
    ...
    if (!duration_id || min_amount === undefined || !coefficient) {
      return NextResponse.json(
        { error: 'Les champs duration_id, min_amount et coefficient sont requis' },
        { status: 400 }
      );
    }

    // Get current coefficient to check overlaps
    const { data: currentCoefficient } = await serviceClient
    ...
    if (!currentCoefficient) {
      return NextResponse.json({ error: 'Coefficient introuvable' }, { status: 404 });
    }

    const leaserIdToUse = leaser_id || currentCoefficient.leaser_id;

    // Check for overlapping ranges (excluding current coefficient)
    const { data: existingCoefficients } = await serviceClient
      .from('leaser_coefficients')
      .select('*')
      .eq('leaser_id', leaserIdToUse)
    ...
    if (duration) {
      // Recalculate prices for non-validated orders (not delivered or cancelled)
...
export async function DELETE(
  ...
  try {
    ...
    // Get the coefficient before deletion to know which leaser and duration to recalculate
    const { data: deletedCoefficient } = await serviceClient
      .from('leaser_coefficients')
      .select('leaser_id, duration_id')
      .eq('id', params.id)
      .single();
    ...
    if (deletedCoefficient) {
      const { data: duration } = await serviceClient
        .from('leasing_durations')
        .select('months')
        .eq('id', deletedCoefficient.duration_id)
        .single();
