...
export default function TimelineTab({ organizationId }: TimelineTabProps) {
  ...
  const fetchLogs = async () => {
    try {
      const response = await fetch(`/api/admin/customers/${organizationId}/logs`);
      if (response.ok) {
        ...
        <div className="p-6">
          {logs.length === 0 ? (
            <div className="text-center py-12 text-gray-500">
              Aucune action enregistrée pour cette organisation
            </div>
          ) : (
