...
export default function NewLeaserPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/admin/leasers', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="container mx-auto bg-gray-50 px-4 py-6 lg:px-6 lg:py-8">
      <div className="mb-6">
        ...
        <h1 className="text-2xl lg:text-3xl font-bold text-black">Ajouter un leaser</h1>
      </div>
      ...
      <div className="max-w-2xl">
        <form onSubmit={handleSubmit} className="rounded-lg bg-white border border-gray-200 p-6 shadow-sm">
          ...
          <div className="mt-8 flex flex-col sm:flex-row gap-4">
            ...
            >
              {loading ? 'Création...' : 'Créer le leaser'}
            </Button>
            <Button
              type="button"
              onClick={() => router.back()}
