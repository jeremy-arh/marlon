 ...
 * via le service client pour garantir que is_super_admin est bien enregistré
 * dans la table user_roles (bypass RLS).
 */
export async function POST(request: NextRequest) {
  try {
    ...
    // Récupérer l'invitation (service client pour bypass RLS)
    let inv: { id: string; organization_id: string; role: string; email: string; is_super_admin?: boolean } | null = null;
    ...
    // Créer le user_role avec is_super_admin (service client = bypass RLS)
    ...
    if (roleError) {
      if (roleError.message.includes('duplicate') || roleError.code === '23505') {
        // Rôle déjà existant : mettre à jour is_super_admin
        await serviceClient
          .from('user_roles')
